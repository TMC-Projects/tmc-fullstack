...
class AchievementSheet extends ConsumerStatefulWidget {
  final Map<String, dynamic>? initialData;
  const AchievementSheet({super.key, this.initialData});
  @override
  ConsumerState<AchievementSheet> createState() => _AchievementSheetState();
}

class _AchievementSheetState extends ConsumerState<AchievementSheet> {
  ...
  void initState() {
    ...
    _dateController = TextEditingController(text: widget.initialData?['AchievementDate'] ?? '');
    _descController = TextEditingController(text: widget.initialData?['Description'] ?? '');
  }

  void _submit() {
    setState(() => _isSubmitting = true);
    // Standardize to YYYY-MM-DDT00:00:00Z if it's just YYYY-MM-DD
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text(widget.initialData == null ? 'Add Achievement' : 'Edit Achievement', style: GoogleFonts.inter(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.white)),
              IconButton(icon: const Icon(LucideIcons.x, color: Color(0xFF94A3B8)), onPressed: () => Navigator.of(context).pop())
            ],
          ),
          const SizedBox(height: 16),
          _buildField('Title (e.g. MVP 2023)', _titleController),
          _buildField('Date (YYYY-MM-DD)', _dateController, hint: '2023-01-01'),

...
export default function Sidebar() {
  ...
  const navLinks = [
    ...
    {
      name: tSidebar('teams', { defaultMessage: 'Tim Squad' }),
      href: '/team-portal/teams',
      icon: Shield,
      roles: ['team_owner']
    },
    {
      name: tSidebar('free_agents', { defaultMessage: 'Free Agents' }),
      href: '/team-portal/player-freeagent',
      icon: Users,
      roles: ['team_owner']
    },
    {
      name: tSidebar('subscriptions', { defaultMessage: 'Subscriptions' }),
      href: '/team-portal/subscriptions',
      icon: Building2,
      roles: ['team_owner']
    }
  ];
  ...
  const renderSidebarContent = (collapsed: boolean) => (
    <div className="flex flex-col h-full bg-slate-50/80 dark:bg-slate-950/80 backdrop-blur-xl border-r border-slate-300 dark:border-slate-800 transition-all duration-300">
      ...
      <div className={`h-16 flex items-center ${collapsed ? 'justify-center px-0' : 'justify-between px-6'} border-b border-slate-300 dark:border-slate-800 shrink-0 overflow-hidden transition-all duration-300`}>
        <Link href="/team-portal/dashboard" className={`flex items-center gap-3 ${collapsed ? 'justify-center' : ''}`}>
          <div className="w-8 h-8 rounded-xl bg-emerald-500/20 flex items-center justify-center text-emerald-500 shrink-0">
            <Building2 className="w-5 h-5" />
          </div>
          {!collapsed && (
            <span className="text-lg font-bold bg-gradient-to-r from-emerald-400 to-teal-500 bg-clip-text text-transparent tracking-wide truncate transition-opacity duration-300">
              Team Portal
            </span>
          )}
        </Link>
    ...
    <>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-slate-50/80 dark:bg-slate-950/80 backdrop-blur-md border-b border-slate-300 dark:border-slate-800 z-40 flex items-center justify-between px-4">
        <Link href="/portal/dashboard" className="flex items-center gap-2">
          <img src="/logo.png" alt="NJARA Logo" className="w-6 h-6 rounded-lg object-contain" />

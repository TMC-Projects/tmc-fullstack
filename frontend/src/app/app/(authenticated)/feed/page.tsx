...
export default function FeedPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setError(err.message || t('failed_fetch'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const fetchFeed = async () => {
    try {
      ...
      if (res.ok && data.success) {
        ...
        throw new Error(data.message || t('failed_fetch'));
      }
    } catch (error) {
      console.error(error);
      throw error;
  ...
  const handleCreatePost = async (content: string) => {
    try {
      ...
      const data = await res.json();
      if (res.ok && data.success) {
        showAlert(t('post_created'), 'success');
        setIsCreateModalOpen(false); // Close modal on success
        fetchFeed(); // Refresh the feed
      } else {
        showAlert(data.message || t('error_creating_post'), 'error');
      }
    } catch (error) {
      showAlert(t('error_creating_post'), 'error');
    }
  };
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" onClick={() => setIsCreateModalOpen(false)}>
              ...
              >
                <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-800">
                  <h2 className="text-xl font-bold px-2">{t('create_post')}</h2>
                  <button
          ...
          <div className="space-y-6">
            {posts.length > 0 ? (
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-12 text-center shadow-sm">
                ...
                <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2">{t('no_posts_title')}</h3>
                <p className="text-slate-500">{t('no_posts_desc')}</p>
              </div>
            )}
          </div>
        </div>

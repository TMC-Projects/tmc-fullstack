...
export default function SubscriptionsPage() {
  ...
  const t = useTranslations('TeamSubscriptions');
  ...
  const fetchPlans = useCallback(async () => {
    try {
      const teamPlans = [
        {
          ...
          price: 9,
          discount: 0,
          description: "Monthly subscription for Team Portal",
          is_active: true,
        ...
        },
        {
          ...
          price: 49,
          discount: 0,
          description: "Yearly subscription for Team Portal",
          is_active: true,
          created_at: new Date().toISOString()
        }
      ];
      ...
      console.error("Failed to set plans", err);
    }
  }, []);
  ...
  useEffect(() => {
    if (_hasHydrated) {
      ...
      } else if (user.category !== 'team_owner') {
        ...
        {activeSub && (
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 md:p-8 shadow-xl border border-slate-200 dark:border-slate-800 flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex items-start gap-5">
              <div className={`p-4 rounded-2xl flex-shrink-0 ${activeSub.status === 'paid' ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-900/40 dark:text-emerald-400' :
              ...
              {plans.map((plan, index) => (
                <div key={plan.id} className={index === 1 ? 'md:-mt-4 md:mb-4 relative z-10' : ''}>
                  <SubscriptionCard
                    ...
                    currency="$"
                    translationNamespace="TeamSubscriptions"
                  />
                </div>
              ))}
            </div>
